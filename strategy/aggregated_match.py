...
class AggregatedMatchStrategy(object):

    def __init__(self):
        super(AggregatedMatchStrategy, self).__init__()
        self.__clear()
    ...
    def __process(self):
        ...
        for idx in xrange(n):
            ...
            if len(result):
                new_match = {'for':self._pipeline['remaining'][idx][0],'match':','.join(t[1] for t in result), 'possiblities':[]}
